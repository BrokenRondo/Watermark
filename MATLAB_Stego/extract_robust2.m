function src_msg_vec=extract_robust2(marked_matrix,src_matrix,len,ii,jj,logistic)
...
for i=1:ii:size(marked_matrix,1)
    for j=1:jj:size(marked_matrix,2)
        ...
        extract_matrix(end+1)=QIM_OUT(marked_matrix(i,j),10);
        %if Dif>=strength/2
